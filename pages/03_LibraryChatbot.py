...
# 2) 도서관 규정 텍스트 ----------------------------------------
# ▶▶ 여기에 '국립부경대학교 도서관 규정' 전체를 복사해서 붙여넣기 ◀◀
LIB_RULES = """ 제1조(목적) 이 규정은 「대학도서관진흥법」과 「국립부경대학교 학칙」(이하 “학칙”이라 한다) 제11조의3에 따라 국립부경대학교 도서관(이하 “도서관”이라 한다)의 발전계획 수립과 진행, 직원의 배치, 학술정보자료의 확보와 효율적인 이용 및 관리에 관한 사항을 규정함을 목적으로 한다. 
...
# 3) 질문 입력 UI ----------------------------------------------
st.write("도서관 규정에 대해 궁금한 점을 물어보세요.")
st.caption("예: 도서관 휴관일은 언제인가요? / 학부생 책 대여 권수는 몇 권인가요?")

# 간단한 1문 1답 형식으로만 만들자 (대화 히스토리 X)
...
if st.button("질문 보내기"):
    if not question.strip():
        ...
        with st.spinner("규정집을 확인하는 중입니다..."):
            # Responses API에 규정 + 질문 전달
            system_prompt = f"""
너는 국립부경대학교 도서관 규정 안내 챗봇이다.
아래에 제공된 도서관 규정 텍스트만을 바탕으로 한국어로 대답해라.

만약 규정에 관련 내용이 전혀 없다면
"해당 내용은 현재 도서관 규정에 명시되어 있지 않습니다." 라고 답해라.
